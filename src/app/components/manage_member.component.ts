...
export class manage_memberComponent {
  ...
  sd_AiUBndLf32UlYQ52(bh) {
    try {
      ...
      bh = this.sd_ZkoMe1TOJ1nIJa3s(bh);
  ...
  sd_ZkoMe1TOJ1nIJa3s(bh) {
    try {
      const page = this.page;
      page.userfromSessionstorage = JSON.parse(sessionStorage.getItem('user'));
      console.log(page.userfromSessionstorage, 'SEssion');
      bh = this.sd_xWJzik1gZbrCcdEL(bh);
      //appendnew_next_sd_ZkoMe1TOJ1nIJa3s
      return bh;
    } catch (e) {
      return this.errorHandler(bh, e, 'sd_ZkoMe1TOJ1nIJa3s');
    }
  }
